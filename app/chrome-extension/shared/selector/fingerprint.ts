 ...
 *
 * Fingerprints support fuzzy element matching and validation, in particular for:
 * - verifying that a selector matched the element we actually expected
 * - restoring elements after HMR
 * - preventing "same selector, different element" mismatches
 */
...
/**
 * Verify that an element matches the given fingerprint
 *
 * Rules:
 * - tag must match exactly
 * - if the stored fingerprint has an id, the current element's id must match
 * - class and text are not required to match (they feed the similarity score)
 *
 * @example
 * ```ts
 ...
 *
 * @returns a similarity score from 0-1, where 1 means an exact match
 *
 * @example
 * ```ts
 * const score = fingerprintSimilarity(fpA, fpB);
 * if (score > 0.8) {
 *   // Highly similar, probably the same element
 * }
 * ```
 */
export function fingerprintSimilarity(a: string, b: string): number {
  ...
  // Class match (weight 0.2) - uses Jaccard similarity
...
/**
 * Check whether two fingerprints refer to the same element
 *
 * Decided by a similarity threshold, default 0.7
 */
