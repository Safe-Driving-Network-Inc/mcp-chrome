/**
 * DOM Path - DOM path computation and lookup
 *
 * A DOM path is an element's index path within the DOM tree. It is used for:
 * - tracking an element's position
 * - fast recovery after a selector goes stale
 * - comparing and validating elements
 */
...
/**
 * DOM path: an array of child indices from the root down to the target element
 *
 * @example
 * ```
 * [0, 2, 1] means:
 ...
 * recording the index within the parent's children at every level.
 *
 * @example
 * ```ts
 ...
 *
 * @param root - the query root node (Document or ShadowRoot)
 ...
 *
 * @returns whether they are identical, plus the length of their common prefix
 *
 * @example
 * ```ts
