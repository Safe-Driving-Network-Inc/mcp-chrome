 ...
 * Locates a DOM element from a list of candidate selectors
 */
...
export class SelectorLocator {
  ...
  async locate(
    ...
    // Extract the fingerprint validation config
    ...
    if (typeof target.selector === 'string' && target.selector.trim()) {
      ...
      if (ensured) {
        ...
        // Fingerprint check: on mismatch, skip and keep trying other candidates
        const fingerprintOk =
          !fingerprintToVerify ||
...
/**
 * Create the selector locator for the Chrome extension
 */
