    ...
    try {
        ...
        try {
          ...
          if (!useOverlay) {
            for (const v of vars) {
              ...
              const def = v.default || '';
              const promptText = `Enter a value for ${label} (${key})`;
          ...
          const title = document.createElement('div');
          title.textContent = 'Enter the replay parameters';
      ...
      if (request && request.action === 'verifyFingerprint') {
        try {
          ...
          // Verify the fingerprint: parse the stored fingerprint and compare it with the current element
