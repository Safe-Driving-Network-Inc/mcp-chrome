/**
 * Output Sanitizer - redaction and length-capping helpers for tool output
 *
 * Safely post-processes JavaScript execution results by:
 * 1. Redacting sensitive data (cookie / token / password, etc.)
 * 2. Capping output length (50KB by default)
 * 3. Deep-serializing objects
 */
...
// Sensitive key identifiers (these get redacted)
// Mirrors the sensitive-key list in mcp-tools.js
...
/**
 * Redact and length-cap an arbitrary value
 */
...
/**
 * Redact sensitive data inside a string
 * Follows the redaction logic in mcp-tools.js, plus Base64/Hex/cookie-query detection
 */
export function sanitizeText(text: string): { text: string; redacted: boolean } {
  ...
  // Cookie / query-string shape detection (contains = plus ; or &)
  if (out.includes('=') && (out.includes(';') || out.includes('&'))) {
    // Detect a cookie string
    if (looksLikeCookieString(out)) {
      return { text: '[BLOCKED: Cookie/query string data]', redacted: true };
    }
    // Detect a query string (key=value&key2=value2 shape)
  ...
  // Hex credential detection (32+ chars of pure hexadecimal)
  ...
  // 4. Sensitive values inside URL query parameters
  replace(
  ...
  // 7. Embedded long hex strings (possibly an API key, hash, etc.)
