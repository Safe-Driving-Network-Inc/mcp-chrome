...
async function getCachedModelData(modelUrl: string): Promise<ArrayBuffer> {
  ...
  try {
    ...
    // 3. Fetch the data and store it in the cache
    ...
    // On failure, clear the possibly-incomplete cache entry
...
export class SemanticSimilarityEngine {
  ...
  private _setupWorker(): void {
    ...
    // Option 1: Chrome extension URL (recommended, most reliable in production)
    try {
    ...
    this.worker.onerror = (error: ErrorEvent) => {
      ...
      if (error.error) {
        // Check whether event.error is present
  ...
  /**
   * Initialization with a progress callback
   */
  ...
  /**
   * Internal initialization with a progress callback
   */
  private async _doInitializeWithProgress(
    ...
    try {
      ...
      // Detect the environment and pick a mode
      const workerSupported = this.isWorkerSupported();
      const inOffscreenDocument = this.isInOffscreenDocument();

      // 🛠️ Avoid an infinite loop: when already inside the offscreen document, force direct Worker mode
      ...
      if (this.useOffscreen) {
        // Offscreen mode - delegate to the offscreen document, which reports its own progress
        ...
        // Make sure the config object serializes correctly by setting every property explicitly
        const configToSend = {
          modelIdentifier: this.config.modelIdentifier,
          localModelPathPrefix: this.config.localModelPathPrefix,
          ...
          useLocalFiles: Boolean(this.config.useLocalFiles), // coerce to a boolean
        ...
        // Round-trip through JSON to guarantee the data is intact
        ...
        // Direct Worker mode - here we can report real progress
    ...
    try {
      ...
      if (this.useOffscreen) {
        ...
        const response = await chrome.runtime.sendMessage({
          target: 'offscreen',
          type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_INIT,
          config: serializedConfig, // use the original config, do not override useLocalFiles
        });
        ...
        if (this.config.useLocalFiles) {
          // Under WXT, assets in public/ live at the root path at runtime
          // Use the model identifier directly; transformers.js prepends /models/ automatically
  ...
  private async _initializeDirectWorkerWithProgress(
    ...
    // Use the progress-callback support in transformers.js 2.17+
    ...
    try {
      ...
      // If the progress callback is unsupported, fall back to the standard path
      console.log(
  ...
  public async warmupModel(): Promise<void> {
    ...
    // More representative warm-up text, covering a range of lengths
    const warmupTexts = [
      // Short text
      'Hello',
      'Hello',
      ...
      'This is a longer passage made up of several sentences. It helps warm the model up for a variety of text lengths.',
    ];

    try {
      // Progressive warm-up: single first, then batched
      ...
      // Keep the warm-up results, do not clear the cache
  ...
  private async _tokenizeText(text: string | string[]): Promise<TokenizedOutput> {
    ...
    // For a single text, try the cache
    if (typeof text === 'string') {
      ...
      // For models that do not need token_type_ids, set return_token_type_ids to false explicitly
      if (!this.config.requiresTokenTypeIds) {
    ...
    // For batches, process directly (batches are rarely repeated)
    const startTime = performance.now();
    const tokenizerOptions: any = {
      padding: true,
      truncation: true,
  ...
  private _extractEmbeddingFromWorkerOutput(
    ...
    // Optimization: use Float32Array directly and skip the needless conversion
    ...
    // Take the embedding array from the memory pool
  ...
  public async getEmbedding(
    ...
    // In offscreen mode, delegate to the offscreen document
    if (this.useOffscreen) {
      const response = await chrome.runtime.sendMessage({
        target: 'offscreen',
        type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_COMPUTE,
  ...
  public async getEmbeddingsBatch(
    ...
    if (this.useOffscreen) {
      ...
      // If everything is cached, return right away
      if (uncachedTexts.length === 0) {
        return results as Float32Array[];
      }

      // Only request the texts that are not cached
      const response = await chrome.runtime.sendMessage({
        target: 'offscreen',
        type: OFFSCREEN_MESSAGE_TYPES.SIMILARITY_ENGINE_BATCH_COMPUTE,
        texts: uncachedTexts,
      ...
      // Put the results back in their original slots and cache them
  ...
  public async computeSimilarityMatrix(
    ...
    // Use the SIMD-optimized matrix maths when available
  ...
  public cosineSimilarity(vecA: Float32Array, vecB: Float32Array): number {
    ...
    if (this.useSIMD && this.simdMath) {
      try {
        // The SIMD version is async, but this method must stay synchronous for interface compatibility,
        // so fall back to the JavaScript version here (or consider refactoring to async)
  ...
  // Added: clear the worker buffers
