...
// Shadow-type effects (Drop Shadow / Inner Shadow)
...
// Blur-type effects (Layer Blur / Backdrop Blur)
...
// A raw effect that could not be parsed
...
export function createEffectsControl(options: EffectsControlOptions): DesignControl {
  ...
  // Per-element effect item cache (scoped to the current editing session only)
  // Why a WeakMap:
  // 1. A hidden effect is not written to CSS (enabled=false), but must be remembered within the session so it can be restored
  // 2. A WeakMap frees memory automatically when the element is removed, so no manual cleanup is needed
  // 3. We only read inline style (never computed), so the cache exists purely to preserve the user's hide actions
  ...
  if (headerActionsContainer) {
    // Put the + button on the right side of the group header (left of the chevron)
    headerActionsContainer.insertBefore(addBtn, headerActionsContainer.firstChild);
    disposer.add(() => addBtn.remove());
  } else {
    // Fallback: show the toolbar in the content area
  ...
  function isEditing(): boolean {
    ...
    // Avoids an over-broad focus check that would stop external style changes from syncing
  ...
  function closePopover(opts?: { commit?: boolean; rollback?: boolean }): void {
    ...
    // Sync once after closing so currentItems matches the real inline style
    // Avoids drift after the browser normalizes or corrects a value
    if (wasOpen && !rollback) {
      syncFromTarget(true);
    }
  }
  ...
  // Listen on document's capture phase so clicks outside the Effects control also close it
  ...
  // Listen for click events on document during the capture phase
  ...
  // Listen for keyboard events on root during the capture phase
