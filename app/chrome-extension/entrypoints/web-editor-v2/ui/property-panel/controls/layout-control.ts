...
function createDisplayIcon(value: DisplayValue): SVGElement {
  ...
  // Container border (dashed rectangle stands for the container)
  ...
  switch (value) {
    case 'block':
      ...
      break;
    case 'inline':
      // Three lines of text stand for inline flow
      addLine(3.5, 4.5, 8);
      addLine(3.5, 7.5, 5);
      addLine(3.5, 10.5, 6.5);
      break;
    case 'inline-block':
      // One block on the left, two lines of text on the right
...
function createHorizontalAlignIcon(value: AlignmentAxisValue): SVGElement {
  ...
  // The content block's X coordinate depends on the alignment
  const blockX: Record<AlignmentAxisValue, number> = {
    'flex-start': 3.5, // align left
    center: 5.5, // align center
    'flex-end': 7.5, // align right
  };

  // Two small squares stand for the child elements (horizontal layout becomes vertical)
...
function createVerticalAlignIcon(value: AlignmentAxisValue): SVGElement {
  ...
  // The content block's Y coordinate depends on the alignment
  const blockY: Record<AlignmentAxisValue, number> = {
    'flex-start': 3.5, // align top
    center: 5.5, // align middle
    'flex-end': 7.5, // align bottom
  };

  // Two small squares stand for the child elements
  const block1 = document.createElementNS(SVG_NS, 'rect');
