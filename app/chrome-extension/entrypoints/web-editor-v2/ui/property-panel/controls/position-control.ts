...
function createPositionIcon(position: PositionValue): SVGElement {
  ...
  switch (position) {
    case 'static':
      // Three horizontal lines stand for normal document flow
      addLine(3.5, 4.5, 8);
      addLine(3.5, 7.5, 8);
      addLine(3.5, 10.5, 8);
      break;
    ...
    case 'relative': {
      // Dashed box is the original position, solid block is the offset position
      ...
      svg.append(ghost);
      // The solid block after the offset
    ...
    case 'absolute':
      // Positioning guide lines (from the container edge to the element)
      addPath('M3 5.5H6M8 3V6', '0.8');
      // Element block in the bottom-right corner
      addBlock(6, 6, 5, 5);
      break;

    case 'fixed': {
      // Pin shape stands for fixed positioning
...
function createFlipXIcon(): SVGElement {
  ...
  // Left mirrored block (semi-transparent = the original)
  ...
  // Right mirrored block (solid = after the flip)
...
function createFlipYIcon(): SVGElement {
  ...
  // Upper mirrored block (semi-transparent = the original)
  ...
  // Lower mirrored block (solid = after the flip)
