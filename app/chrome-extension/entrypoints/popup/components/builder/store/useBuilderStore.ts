...
export function useBuilderStore(initial?: FlowV2 | null) {
  ...
  function setNodePosition(id: string, x: number, y: number) {
    ...
    // Not recorded in the history stack to avoid frequent entries; user-triggered actions (connect/add/delete, etc.) are recorded instead.
  }
  ...
  function onConnect(sourceId: string, targetId: string, label: string = 'default') {
    ...
    if (sourceId === targetId) {
      toast('Cannot connect to itself', 'warn');
      return;
    }
    // IO constraints
    try {
      ...
      if (dstIo.inputs !== 'any' && incoming >= (dstIo.inputs as number)) {
        toast(`This node allows at most ${dstIo.inputs} incoming edges`, 'warn');
        return;
      }
      ...
      if (srcIo.outputs !== 'any') {
        ...
        if (outgoing >= (srcIo.outputs as number)) {
          toast(`This node allows at most ${srcIo.outputs} outgoing edges`, 'warn');
          return;
        }
      }
    } catch {}
    // Single outgoing edge per label: remove existing edges with the same source + same label
    for (let i = edges.length - 1; i >= 0; i--) {
      const e = edges[i];
  ...
  // Fallback layout: layering + barycenter ordering (no external library required)
  ...
  // Auto layout (ELK preferred):
  // - Dynamically import elkjs to avoid a permanent bundle size cost
  // - Fall back to layoutFallback() on failure
  async function layoutAuto() {
    try {
      ...
      try {
        layoutFallback();
        toast('ELK auto layout unavailable, using fallback layout', 'warn');
      } catch {}
    }
  }
