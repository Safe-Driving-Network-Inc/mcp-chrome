<template>
  ...
  <aside class="property-panel">
    <div v-if="node" class="panel-content">
      ...
      <!-- Property form: rendered uniformly by the NodeSpec-driven form engine -->
      <PropertyFromSpec
        v-if="node"
    ...
    <div v-else class="panel-empty">
      ...
      <div class="empty-text">Select a node<br />to view and edit properties</div>
    </div>
  </aside>
</template>
...
const extractErrors = computed(() => {
  ...
  const errs: string[] = [];
  if (!n.config?.saveAs) errs.push('A save variable name is required');
  if (!n.config?.selector && !n.config?.js) errs.push('Provide either a selector or js');
  return errs;
});
...
// Highlight and scroll to the specified field
watch(
