<template>
  ...
  <div class="builder-page rr-theme" :data-theme="theme">
    <div v-if="fallbackNotice" class="notice-top">
      <span>Fallback suggestion applied: promoted {{ fallbackNotice.type }} priority</span>
    ...
    <div class="main">
      ...
      <div class="topbar rr-topbar backdrop-blur">
        <div class="left">
          ...
          <span class="tip">Visual workflow builder</span>
        </div>
        <div class="right">
          ...
          <button
            ...
            @click="runAll"
            title="Replay the whole flow from the start"
          >
            <svg
              width="14"
              height="14"
  ...
  if (q.flowId) {
    try {
      ...
      if (flowV3) {
        ...
        pushToast(`Workflow "${q.flowId}" not found; a new workflow was created`, 'warn');
        initEmptyFlow();
      }
    } catch (e) {
 ...
 * @returns the cron expression, or null if it cannot be converted
 */
...
/**
 * Sync triggers from trigger-node configs into V3 storage
      ...
      cfg.schedules.forEach((s: any, i: number) => {
        ...
        if (!cron) {
          ...
          if (scheduleType === 'once') {
            pushToast(
              `Schedule #${i + 1} on node ${n.id}: V3 does not yet support one-time (once) schedules; skipped`,
              'warn',
            );
          } else {
            pushToast(
              `Schedule #${i + 1} on node ${n.id}: cannot convert to cron (type=${scheduleType}); skipped`,
              'warn',
            );
          }
          return;
