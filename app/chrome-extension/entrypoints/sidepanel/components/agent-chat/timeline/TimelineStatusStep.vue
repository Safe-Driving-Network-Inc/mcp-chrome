<template>
  <div class="flex items-center gap-2">
    <!-- Spiral loading icon (shown only in running/starting state, unless hidden by parent) -->
    <svg
...
// Whether the step is in a running state
const isRunning = computed(
  () => props.item.status === 'running' || props.item.status === 'starting',
);

// Random text (used only in the running state)
const randomText = ref(getRandomLoadingText());

// Timeout ID for periodically updating the text
...
function startInterval(): void {
  if (timeoutId) return;
  // Update text at a random 5-8 second interval
  const scheduleNext = () => {
...
watch(isRunning, (running) => {
  // Only regenerate text and start the timer when going from not-running to running
