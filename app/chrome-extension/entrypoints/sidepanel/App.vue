          ...
          <div class="em-stats-bar">
            <span class="em-stats-text">
              <template v-if="markerSearch">
                <strong>{{ filteredMarkers.length }}</strong> markers matched (of
                {{ markers.length }} total, {{ groupedMarkers.length }} domains)
              </template>
              <template v-else>
                <strong>{{ markers.length }}</strong> markers,
...
function createTrigger() {
  ...
  alert('V3 trigger management is not implemented yet, cannot create triggers for now');
}

function editTrigger(_id: string) {
  // V3 Trigger management not yet implemented
  alert('V3 trigger management is not implemented yet, cannot edit triggers for now');
}
...
function edit(id: string) {
  ...
  alert('V3 Builder is not implemented yet, cannot edit workflows for now');
  // TODO: openBuilder({ flowId: id });
}

function createFlow() {
  // V3 Builder not yet implemented - show message
  alert('V3 Builder is not implemented yet, cannot create workflows for now');
  // TODO: openBuilder({ newFlow: true });
}

async function remove(id: string) {
  try {
    const ok = confirm('Delete this workflow? This action cannot be undone.');
    if (!ok) return;
