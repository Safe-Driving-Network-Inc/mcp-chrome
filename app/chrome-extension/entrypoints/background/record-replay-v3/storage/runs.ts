 ...
 * @description Implements the CRUD operations for run records
 */
...
/**
 * Validate a run record structure
 */
...
/**
 * Create the RunsStore implementation
 */
    ...
    async patch(id: RunId, patch: Partial<RunRecordV3>): Promise<void> {
      return withTransaction(RR_V3_STORES.RUNS, 'readwrite', async (stores) => {
        ...
        const updated: RunRecordV3 = {
          ...existing,
          ...patch,
          id: existing.id, // make sure the id never changes
          schemaVersion: existing.schemaVersion, // make sure the version never changes
          updatedAt: Date.now(),
        };
