 ...
 * @description Converts V2-format data into V3 format, in both directions
 */
 ...
 * @param v2Flow the Flow in V2 format
 * @returns the conversion result: success/failure, the data, and any errors or warnings
 */
export function convertFlowV2ToV3(v2Flow: V2Flow): ConversionResult<FlowV3> {
  ...
  // Bail out immediately on a fatal error
  if (errors.length > 0) {
...
/** entryNodeId computation result */
 ...
 *
 * Rules:
 * 1. Exclude trigger nodes (they are UI-only and do not execute)
 * 2. Compute in-degree from "executable node -> executable node" edges only (ignore edges leaving a trigger)
 * 3. Nodes with in-degree 0 become the candidates
 * 4. With several candidates, apply a stable selection rule:
 *    - Prefer the node closest to the top-left in UI coordinates (x ascending, then y ascending)
 *    - With no UI coordinates, take the first by lexicographic ID order
 */
function findEntryNodeId(nodes: NodeV3[], edges: EdgeV3[]): EntryNodeResult {
  const warnings: string[] = [];

  // 1. Exclude trigger nodes to get the executable nodes
  ...
  // 2. Compute in-degree (counting only edges between executable nodes)
  const inDegree = new Map<NodeId, number>();
  for (const node of executableNodes) {
    inDegree.set(node.id, 0);
  }
  for (const edge of edges) {
    // Ignore edges leaving a non-executable node (such as a trigger)
    if (!executableNodeIds.has(edge.from)) {
      continue;
    }
    // Ignore edges pointing at a non-executable node
  ...
  // 3. Find the nodes with in-degree 0
  const rootNodes = executableNodes.filter((n) => inDegree.get(n.id) === 0);

  if (rootNodes.length === 0) {
    // No node has in-degree 0, so the graph contains a cycle; pick a fallback with the stable selector
  ...
  // 4. A single root node, return it directly
  if (rootNodes.length === 1) {
    return { nodeId: rootNodes[0].id, warnings };
  }

  // 5. Several root nodes, apply the stable selection rule
...
/**
 * Pick one stable entry node from several root nodes
 * Prefers UI coordinates (top-left wins), then falls back to lexicographic ID order
 */
function selectStableRootNode(nodes: NodeV3[]): StableSelectionResult {
  // Check whether the node has usable UI coordinates
  ...
  if (nodesWithUi.length > 0) {
    // Sort by UI coordinates: x ascending -> y ascending -> id lexicographic (as the tie-breaker)
...
function convertMetaV2ToV3(v2Meta: V2Flow['meta']): FlowV3['meta'] | undefined {
  ...
  // Return undefined when meta is an empty object
...
/**
 * Convert a V3 Flow to a V2 Flow (so it can be edited in the V2 Builder)
 * @param v3Flow the Flow in V3 format
...
/**
 * Convert a V2 trigger to a V3 TriggerSpec
 * @param v2Trigger the trigger in V2 format
 * @returns the conversion result
 */
export function convertTriggerV2ToV3(v2Trigger: V2Trigger): ConversionResult<TriggerSpec> {
  ...
  // Build a different TriggerSpec depending on the type
...
/**
 * Convert a V2 schedule config into a cron expression
 */
function convertScheduleToCron(schedule: V2Trigger['schedule']): string | null {
  ...
  switch (schedule.type) {
    case 'interval': {
      // Approximate the interval as cron (every N minutes)
    ...
    case 'daily':
      ...
      return '0 0 * * *'; // defaults to daily at 00:00

    case 'weekly': {
      // On given days of the week, at a given time
...
/**
 * Create a NotImplemented V2ToV3Converter (kept for backwards compatibility)
