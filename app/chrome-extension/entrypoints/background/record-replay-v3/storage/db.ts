 ...
 * @description The schema and initialization logic for the rr_v3 database
 */
 ...
 * @description Contains every index needed for phases 1-3, so no later upgrade is required
 */
...
export function handleUpgrade(db: IDBDatabase, oldVersion: number, _newVersion: number): void {
  // Version 0 -> 1: create every store
  if (oldVersion < 1) {
 ...
 * @description A singleton, so there is only ever one database connection
 */
...
/**
 * Run a transaction
 * @param storeNames the store name (or names)
