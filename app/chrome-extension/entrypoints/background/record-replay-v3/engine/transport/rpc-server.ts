...
/**
 * RPC Server
 * @description Handles the RPC requests coming from the UI
 */
export class RpcServer {
  ...
  /**
   * Handle a listQueue request
   * @description Lists the queue items, sorted by priority DESC + createdAt ASC
   */
  ...
  /**
   * Handle a cancelQueueItem request
   * @description Cancels a queued item, updates the run state and publishes run.canceled
   * @note Only items with status=queued may be cancelled; running/paused must use rr_v3.cancelRun
   */
  private async handleCancelQueueItem(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // 2. Only queued items may be cancelled (running/paused must use rr_v3.cancelRun)
    ...
    // 4. Update the run record's state
    ...
    // 5. Publish the run.canceled event (via EventsBus, so it is broadcast)
  ...
  /**
   * Handle a saveFlow request
   * @description Saves or updates a Flow, running full structural validation
   */
  private async handleSaveFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check whether this updates an existing flow (query by the trimmed ID)
    ...
    // Normalize the flow, passing existingFlow so createdAt is inherited
    const flow = this.normalizeFlowSpec(rawFlow, existingFlow);

    // Write to storage (the storage layer validates a second time)
  ...
  /**
   * Handle a deleteFlow request
   * @description Deletes a Flow, first checking for associated triggers and queued runs
   */
  private async handleDeleteFlow(params: JsonObject | undefined): Promise<JsonValue> {
    ...
    // Check for queued runs (runs that have not executed would fail after the delete)
  ...
  /**
   * Normalize a FlowV3 input
   * @description Validates the input and converts it into a complete FlowV3 structure
   * @param value the raw input
   * @param existingFlow the existing flow (used to inherit createdAt)
   */
  private normalizeFlowSpec(value: unknown, existingFlow: FlowV3 | null = null): FlowV3 {
    ...
    // Validate variables: each entry must be an object carrying a name field
  ...
  private normalizeEdge(value: unknown, index: number): EdgeV3 {
    ...
    // Validate or generate the id (non-empty + trim)
    let id: EdgeId;
    if (raw.id === undefined || raw.id === null) {
