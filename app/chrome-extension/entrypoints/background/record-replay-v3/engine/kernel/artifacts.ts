 ...
 * @description The interface for capturing and storing artifacts such as screenshots
 */
...
export interface ArtifactService {
   ...
   * @param base64 the screenshot data
...
export interface ArtifactPolicyExecutor {
  /**
   * Run the screenshot policy
   * @param policy the screenshot policy
...
export function createArtifactPolicyExecutor(service: ArtifactService): ArtifactPolicyExecutor {
  return {
    executeScreenshotPolicy: async (policy, context) => {
      // Let the policy decide whether to take a screenshot
      ...
      // Save (when a filename was given)
      if (context.saveAs) {
