 ...
 * @description
 * Implements one-shot scheduled firing using chrome.alarms' `when` parameter.
 *
 * Behaviour:
 * - One one-shot alarm per trigger
 * - After firing, the trigger is disabled (enabled=false) and uninstalled automatically
 */
...
export interface OnceTriggerHandlerDeps {
  ...
  /**
   * Optional: a custom way to disable the trigger.
   * When omitted, the TriggerStore is updated directly.
   */
...
/**
 * Parse the triggerId out of an alarm name
 */
...
/**
 * Create the once trigger handler factory
 */
...
/**
 * Create the once trigger handler
 */
export function createOnceTriggerHandler(
  ...
  // Create the store lazily, to avoid trouble in test environments
  ...
  /**
   * Bump the version so pending operations become stale
   */
  ...
  /**
   * Internal uninstall logic (does not call the external uninstall)
   */
  ...
  const onAlarm = (alarm: chrome.alarms.Alarm): void => {
    ...
    void (async () => {
      try {
        ...
        // Check that the version is still valid
  ...
  /**
   * Make sure we are listening for alarm events
   */
