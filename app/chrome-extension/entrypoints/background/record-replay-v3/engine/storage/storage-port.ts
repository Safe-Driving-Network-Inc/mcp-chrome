 ...
 * @description The storage-layer abstraction used for dependency injection
 */
 ...
 * @description seq allocation must happen atomically inside append()
 */
export interface EventsStore {
  /**
   * Append an event and atomically allocate its seq
   * @description Within one transaction: read RunRecordV3.nextSeq -> write the event -> increment nextSeq
   * @param event the event input (without seq)
   * @returns the complete event (with the allocated seq and ts)
   */
 ...
 * @description Aggregates every store interface for dependency injection
 */
 ...
 * @description Avoids a Proxy synthesizing 'then', which would make it thenable
 */
