 ...
 * - A global singleton keepalive-manager avoids conflicting controllers
 * - RunExecutor uses RunRunner to actually execute the Flow
 */
...
/**
 * Resolve the Tab ID a run needs
 ...
 * Note: re-reads the latest RunRecord so startedAt is correct
 */
async function failRun(
  ...
  // Re-read the latest run record so startedAt is correct
 ...
 * Executes the Flow via RunRunner
 */
function createDefaultRunExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    // 4. Sync attempt back to the RunRecord
    try {
      await deps.storage.runs.patch(runId, {
    ...
    try {
      ...
      // Register with the RunnerRegistry, for DebugController and RPC
      ...
      // 6. Deregister the runner
      if (runner) {
        deps.runners.unregister(runId);
      }

      // 7. Clean up the temporary tab
      if (shouldClose) {
...
/**
 * Get the current runtime (if it has started)
 */
