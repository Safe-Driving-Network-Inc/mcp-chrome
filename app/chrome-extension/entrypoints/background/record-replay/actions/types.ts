 ...
 * Core type definitions for production-grade record & replay
 *
 * Design principles:
 * - Type safe, no any
 * - Covers every action type
 * - Supports retry, timeout and error-handling strategies
 * - Supports selector candidate lists and stability scoring
 * - Supports the variable system
 * - Follows SOLID (interfaces are extensible via declaration merging)
 */
...
export interface TimeoutPolicy {
  ms: Milliseconds;
  /** 'attempt' = time each attempt separately, 'action' = time the whole action */
...
/**
 * ActionParamsByType is declared as an interface so that external modules
 * can extend the Action type through declaration merging (satisfying OCP)
 */
...
export interface ActionExecutionResult<T extends ActionType = ActionType> {
  ...
  error?: ActionError;
  /** Label of the next edge (used by conditional branches) */
  nextLabel?: EdgeLabel;
  /** Control-flow instruction (foreach/while) */
...
export interface ActionHandler<T extends ExecutableActionType = ExecutableActionType> {
  ...
  /** Build the action description (for display in the UI) */
