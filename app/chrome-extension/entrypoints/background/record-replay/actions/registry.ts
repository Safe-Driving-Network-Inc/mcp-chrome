/**
 * Action Registry - action handler registry and execution pipeline
 *
 * Features:
 * - Dynamic handler registration / deregistration
 * - Middleware / hook mechanism (beforeExecute, afterExecute)
...
export class ActionRegistry {
  ...
  /**
   * Register several hooks at once
   */
  ...
  async execute<T extends ExecutableActionType>(
    ...
    // Work out the retry and timeout parameters
    ...
    // Execution loop (supports retries)
    for (let attempt = 0; attempt < maxAttempts; attempt++) {
      ...
      // beforeExecute hooks (may short-circuit)
