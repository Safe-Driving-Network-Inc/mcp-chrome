 ...
 * @description
 * Exercises the full V3 path: RPC -> enqueue -> schedule -> run -> complete
 *
 * These tests use:
 * - Real IndexedDB storage (fake-indexeddb)
 * - Service-level RPC (calling the internal handlers directly, avoiding a Port mock)
 */
...
describe('V3 service-level E2E', () => {
  ...
  describe('Event streaming', () => {
    ...
    it('subscribe with runId filter only receives events for that run', async () => {
      ...
      // Subscribe to runId1's events only (but runId1 has already completed)
      ...
      // We should not receive run2's events
  ...
  describe('Control plane', () => {
    it('pause/resume: pauseRun marks queue paused, resumeRun completes succeeded', async () => {
      ...
      // Enable pauseOnStart at enqueue time
      const { runId } = await client.call<{ runId: string }>('rr_v3.enqueueRun', {
        flowId: flow.id,
    ...
    it('cancel queued run removes it from queue', async () => {
      // Create a fresh harness that does not auto-start the scheduler
      await h.dispose();
      ...
      // The run state should be canceled
  ...
  describe('Recovery', () => {
    it('orphan running lease is requeued and run can complete', async () => {
      // Stop the current harness and create one that does not start the scheduler
      await h.dispose();
      ...
      // The queue item should be back in the queued state
      ...
      // Start the scheduler; the run should carry on executing
      h.scheduler.start();
    ...
    it('cleans terminal runs left in queue', async () => {
      ...
      // Simulate a crash: the run completed but the queue item was never cleaned up
      await h.storage.queue.enqueue({ id: runId, flowId: flow.id, priority: 0 });
      await h.storage.queue.markRunning(runId, 'owner-old', Date.now());
