 ...
 * @description A simple test node supporting success, failure and delay
 */
...
/**
 * Create the V3 E2E test harness
 * @description Assembles the complete V3 runtime for integration testing
 */
export function createV3E2EHarness(options: V3E2EHarnessOptions = {}): V3E2EHarness {
  ...
  // 6) RunExecutor - wires the scheduler to the runner
  ...
  async function waitForEvent(
    ...
    // Fast path: check the already-persisted events
    try {
...
/**
 * Create the RunExecutor for E2E tests
 */
function createE2EExecutor(deps: {
  ...
  return async (item: RunQueueItem): Promise<void> => {
    ...
    // 3. Sync attempt/tabId back to the RunRecord
